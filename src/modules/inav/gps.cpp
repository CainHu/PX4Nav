...
//            _eskf.reset_covariance_matrix<1>(2, sq(_eskf._params.gps_pos_vert_noise));
    ...
    void GpsHgtAidingInterface::anomaly_detection() {
        ...
        if (!gps->_inav->_fault_status.flags.bad_acc_vertical && _actived) {
            ...
            if (gps->_inav->is_recent(time_last_fuse, RunnerParameters::HGT_FUSE_TIMEOUT)) {
                cout << "gps->_inav->is_recent(time_last_fuse, RunnerParameters::HGT_FUSE_TIMEOUT)" << endl;
            }
        }
...
//            _buffer.clear();
            ...
            if (gps->_inav->is_timeout(time_last_fuse, RunnerParameters::HORZ_FUSE_TIMEOUT)) {
                _actived = false;
            }

            cout << "gps horz is not anomaly" << endl;
        }
    ...
    void GpsVelHorzAidingInterface::anomaly_detection() {
        ...
        if (inav->_control_status.flags.tilt_align) {
            if (_actived) {
                ...
                _anomaly = gps->_checks_failing && is_other_source_of_horizontal_velocity_aiding_than_gps;
