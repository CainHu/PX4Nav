...
//        std::cout << "set_gps_time = " << gps_sample.time_us << std::endl;
    ...
    template<uint8_t DELAYS>
    template<uint8_t N>
    bool ESKFRunner<DELAYS>::collect_gps(const GpsMessage &gps) {
        ...
        if (_filter_initialised && !_NED_origin_initialised && _gps_state.checks_passed[N) {
            // If we have good GPS data set the origin's WGS-84 position to the last gps fix
            const double lat = gps.lat * 1.0e-7;
            const double lon = gps.lon * 1.0e-7;

            if (!_pos_ref.isInitialized()) {
                _pos_ref.initReference(lat, lon, _imu_sample_last.time_us);

                // if we are already doing aiding, correct for the change in position since the EKF started navigating
                if (is_horizontal_aiding_active()) {
                    double est_lat;
                    double est_lon;
                    _pos_ref.reproject(-_eskf._state.pos(0), -_eskf._state.pos(1), est_lat, est_lon);
                    _pos_ref.initReference(est_lat, est_lon, _imu_sample_last.time_us);
                }
            }

            // Take the current GPS height and subtract the filter height above origin to estimate the GPS height of the origin
            _gps_alt_ref = 1e-3f * (float)gps.alt + _eskf._state.pos(2);
